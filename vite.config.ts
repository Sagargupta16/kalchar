...
function seoPlugin(): Plugin {
  ...
  return {
    ...
    transformIndexHtml: {
      order: 'pre',
      handler(html) {
        const artworksRaw = readFileSync(
          resolve(__dirname, 'src/data/artworks.json'),
          'utf8',
        );
        const siteRaw = readFileSync(
          resolve(__dirname, 'src/data/site.json'),
          'utf8',
        );
        ...
        const sameAs = [
          siteData.contact.instagram.url,
          siteData.contact.whatsapp.url,
        ].filter(Boolean);
