...
- **Events section** ([app/events/page.tsx](app/events/page.tsx), [components/events/event-gallery.tsx](components/events/event-gallery.tsx)) -- public `/events` page (peacock accent) for workshops held, exhibitions, classes, and gatherings. Each event is a distinct card with a photo arrangement: 1-2 photos tile equally; 3+ show a wide cover hero over a width-filling row, with a "+N more" tile opening an image-only lightbox (keyboard + swipe). Events sort newest-first, with any event pinnable to the top. Graceful empty state. Added to the header nav, footer Explore, and a home "Recent events" preview strip.
